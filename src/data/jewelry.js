const jewelryData = [
    {
    id : 'jewelry1',
    name : '브라운슈가 쥬얼리 밀크티',
    img : '/img/product/jewelry1.jpg',
    desc : '부드럽고 달콤한 브라운 슈가 쥬얼리와 신선한 우유의 맛이 느껴지는 밀크티',
    price : 5500,
    sugar : ['기본'],
    ice : [1]
    },
    {
    id : 'jewelry2',
    name : '브라운슈가 쥬얼리 우롱 스무디',
    img : '/img/product/jewelry2.jpg',
    desc : '부드럽고 달콤한 브라운슈가 쥬얼리에 고소하고 깊은 향의 우롱차 스무디를 더한 스페셜 토핑 메뉴',
    price : 5700,
    sugar : ['기본'],
    ice : [1]
    },
    {
    id : 'jewelry3',
    name : '딸기 쥬얼리 밀크티',
    img : '/img/product/jewelry3.jpg',
    desc : '얼그레이티로 만든 밀크티에 리얼 딸기 과즙이 함유된 새콤달콤한 딸기 쥬얼리를 더한 음료 (딸기 함량 UP!)',
    price : 5300,
    sugar : ['0%','30%','50%','70%','100%'],
    ice : [1]
    },
    {
    id : 'jewelry4',
    name : '딸기 쥬얼리 요구르트 크러쉬',
    img : '/img/product/jewelry4.jpg',
    desc : '얼그레이티를 넣어 만든 요구르트 크러쉬에 리얼 딸기 과즙이 함유된 새콤달콤한 딸기 쥬얼리를 더한 시즌 음료',
    price : 5500,
    sugar : ['기본'],
    ice : [1]
    },
    {
    id : 'jewelry5',
    name : '딸기 쥬얼리 벨벳 밀크티',
    img : '/img/product/jewelry5.jpg',
    desc : '쫀득한 쥬얼리를 넣은 딸기 얼그레이 밀크티에 단짠 치즈폼과 바삭한 딸기 다이스까지 더해 다양한 식감을 느낄 수 있는 프리미엄 밀크티',
    price : 5500,
    sugar : ['0%','30%','50%','70%','100%'],
    ice : [1]
    }
    ]
    
    export default jewelryData